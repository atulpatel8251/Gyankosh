...
logger = logging.getLogger("cache_app")
...
class OCRCache:
    ...
    def list_cached_files(self):
        """List all files in the cache with their original paths"""
        cache_index = self.load_cache_index()
        cached_files = {}
        
        for file_hash, info in cache_index.items():
            file_path = info.get('file_path', 'Unknown')
            cached_files[os.path.basename(file_path)] = {
                'hash': file_hash,
                'full_path': file_path,
...
def get_text_from_cache(pdf_file_path, cache_system):
    """Retrieve text from cache only"""
    logger.info(f"Looking for file in cache: {os.path.basename(pdf_file_path)}")
    
    # Check cache
    cached_text = cache_system.get_cached_text(pdf_file_path)
    if cached_text is not None:
        st.success(f"Found cached text for {os.path.basename(pdf_file_path)}")
        return cached_text
    else:
        logger.warning(f"No cached text found for {os.path.basename(pdf_file_path)}")
        st.warning(f"No cached text found for {os.path.basename(pdf_file_path)}")
        return None

def process_files_from_cache(selected_files, folder_path, progress_bar, status_text):
    """Process multiple files using only the cache"""
    logger.info(f"Starting cache retrieval for {len(selected_files)} files")
    ...
    for i, file in enumerate(selected_files):
        try:
            ...
            # Get text from cache
            text = get_text_from_cache(pdf_file_path, cache_system)
            
            if text and text.strip():
                combined_text.append(text)
                processed_files.append(file)
                logger.info(f"Successfully retrieved from cache: {file}")
            else:
                logger.warning(f"No text found in cache for: {file}")
    ...
    logger.info(f"Cache retrieval complete. Successfully processed {len(processed_files)}/{total_files} files")
                    ...
                    if os.path.exists(subjects_folder):
                        ...
                        if selected_subject and selected_subject != "Select subject":
                            ...
                            if "Select All" in selected_file:
                                selected_files = files_list[1:]  # All except "Select documents"
                            ...
                            if selected_files:
                                ...
                                if combined_text:
                                    ...
                                    with st.expander("Preview of extracted text"):
                                        preview_text = st.session_state.final_text[:1000] + "..." if len(st.session_state.final_text) > 1000 else st.session_state.final_text
                                    ...
                                    if submitted and "final_text" in st.session_state and st.session_state.mode_of_questions != 'Select Option':
                                        try:
                                            # Import ConversationChain and ChatOllama only if needed
                                            from langchain.chains import ConversationChain
                                            from langchain.llms import ChatOllama
                                            
                                            st.session_state.llm = ConversationChain(llm=ChatOllama(model="llama3", temperature=0.7))
                                            chapter_info = f"Chapter: {selected_files[0]}" if len(selected_files) == 1 else "Multiple Chapters"
                                    ...
                                    st.warning("No text found in cache for the selected files.")
                    else:
                        st.error(f"Medium folder not found: {subjects_folder}")
